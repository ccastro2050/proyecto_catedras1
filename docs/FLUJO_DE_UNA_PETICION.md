...
```
GET /api/sede HTTP/1.1          ← un GET: verbo + ruta, sin body
```

```
POST /api/sede HTTP/1.1         ← un POST: verbo + ruta...
...
| `[Route("api/sede")]` | `SedeController.cs` | "Todas mis rutas cuelgan de /api/sede" |
...
| `[FromBody] SedeCrear body` | parámetros del método | Captura el BODY: toma el JSON, lo vuelca en la PETICIÓN del verbo y lo VALIDA |

Cuando llega `GET /api/sede`, el enrutador compara el verbo y la ruta
de la petición contra los atributos de todos los controladores, encuentra
el método `Listar()` (que tiene `[HttpGet]` bajo `[Route("api/sede")]`)
...
`POST /api/sede` con `{"codigo":"PR009","nombre":"Webcam","stock":10,"valorunitario":350000}`:

```
1. ASP.NET routing     compara verbo+ruta contra los atributos
                       → elige SedeController.Crear()
2. Model binding       toma el JSON del body y lo vuelca en la petición SedeCrear
...
4. SedeController  construye la entidad Sede y llama al servicio
5. ServicioSede    delega al repositorio (por la interfaz)
6. RepositorioSedePostgreSql
                       INSERT INTO sede (...) VALUES (@codigo, ...)  ← parametrizado
...
```mermaid
flowchart TD
    A["1. Kestrel recibe<br/>el texto HTTP"] --> B["2. ASP.NET Core encuentra<br/>POST /api/sede<br/>en la tabla de rutas"]
    B --> C{"3. ¿el body cumple las<br/>anotaciones de SedeCrear?<br/>(Required, Range...)"}
...
2. SedeController try { … } → llama al servicio
3. ServicioSede   pide al repositorio; si llega null → lanza
                      NoEncontradoExcepcion (el catch la vuelve 404)
4. Repositorio        SELECT ... WHERE codigo = @codigo
5. La fila vuelve convertida en objeto Sede y sale como JSON
...
```mermaid
flowchart LR
    A["GET /api/<br/>sede/PR001"] --> B["controlador"] --> S["servicio"] --> R["repositorio:<br/>SELECT ... WHERE<br/>codigo = @codigo"]
    R --> E{"¿hay fila?"}
    E -->|"sí"| OK["200: la sede<br/>en JSON"]
...
Invoke-RestMethod "http://localhost:8037/api/sede"
Invoke-RestMethod "http://localhost:8037/api/sede/PR001"

# POST — crear
Invoke-RestMethod -Method Post -Uri "http://localhost:8037/api/sede" -ContentType "application/json" -Body '{"codigo":"PR009","nombre":"Webcam","stock":10,"valorunitario":350000}'

# PUT con body incompleto → error 422 (PUT exige TODO)
Invoke-RestMethod -Method Put -Uri "http://localhost:8037/api/sede/PR009" -ContentType "application/json" -Body '{"stock":25}'

# PATCH con el MISMO body → 200 (PATCH es parcial)
Invoke-RestMethod -Method Patch -Uri "http://localhost:8037/api/sede/PR009" -ContentType "application/json" -Body '{"stock":25}'

# DELETE — limpiar
Invoke-RestMethod -Method Delete -Uri "http://localhost:8037/api/sede/PR009"
```
