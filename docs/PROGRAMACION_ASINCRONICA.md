...
Cuando su API atiende `GET /api/sede`, ¿en qué gasta el tiempo?
...
public async Task<List<Sede>> ObtenerTodosAsync(int limite)
{
...
public async Task<List<Sede>> ObtenerTodosAsync(int limite)
{
    ...
    var filas = await conexion.QueryAsync<Sede>(
