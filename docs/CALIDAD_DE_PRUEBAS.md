...
[Fact]
public async Task CrearSede_Funciona()
{
    var creado = await servicio.CrearSedeAsync(peticion);
    // (sin Assert: si CrearSedeAsync guarda mal, esta prueba PASA)
}
```
...
```csharp
[Fact]
public async Task CrearSede_GuardaElNombre()
{
    var creado = await servicio.CrearSedeAsync(peticion);
