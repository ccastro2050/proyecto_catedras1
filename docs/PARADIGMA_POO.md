...
Problema: saber cuántas fichas activas hay por nombre. Mírelo tres veces:
...
{
    conteo.TryGetValue(f.Nombre, out var n);
    conteo[f.Nombre] = n + 1;
}
```
...
SELECT nombre, COUNT(*) FROM sede
WHERE activo = TRUE GROUP BY nombre;
```

```csharp
// P.O.O.: objetos que colaboran — cada uno con SU responsabilidad
$servicio->contarPorNombre();   // el servicio le PIDE al repositorio;
...
```mermaid
flowchart TB
    subgraph PROY["El proyecto api_catedras — multiparadigma a propósito"]
...
Y en las capas: `RepositorioSedePostgreSql` guarda su cadena de conexión
...
interfaz. `ServicioSede` funciona igual con
`RepositorioSedePostgreSql` (el real) que con `RepositorioFalsoEnMemoria`
(el de la prueba de capas) — porque ambos firman `: IRepositorioSede`.
...
(`IServicioSede`, `IRepositorioSede`) son abstracción pura: declaran
...
classDiagram
    ...
    class RepositorioSedePostgreSql {
    ...
    class ServicioSede {
        -repositorio: IRepositorioSede
        +reglas de negocio (límite mayor que 0, ...)
    }
    RepositorioSedePostgreSql ..|> IRepositorioSede : POLIMORFISMO
    RepositorioFalsoEnMemoria ..|> IRepositorioSede : POLIMORFISMO
    ServicioSede o-- IRepositorioSede : COMPOSICIÓN (recibe, no hereda)
    note for IRepositorioSede "ABSTRACCIÓN: declara QUÉ — ni una línea de CÓMO"
    note for RepositorioSedePostgreSql "ENCAPSULAMIENTO: la conexión y el SQL no salen de aquí"
```
...
{
    Task<bool> CrearAsync(Sede datos);
}
...
{
    public Task<bool> CrearAsync(Sede datos)
...
Cuando `ServicioSede` manda el mensaje `crear(datos)`, NO sabe (ni le
...
classDiagram
    direction LR
    class ServicioMal["ServicioSede ❌"]
    class ServicioBien["ServicioSede ✅"]
    ServicioMal --|> RepositorioSedePostgreSql : hereda del CONCRETO — quedó casado con SQL Server
    ServicioBien o-- IRepositorioSede : compone la ABSTRACCIÓN — cualquier motor entra
...
sequenceDiagram
    ...
    participant C as SedeController
    participant S as ServicioSede
    participant R as IRepositorioSede (interface)
    participant BD as SQL Server
    Cli->>C: POST /api/sede (JSON)
...
> modelos viven en `Modelos/`; en la v1 hay uno: `Sede`.
...
| En la v1 | `Sede` | `SedeCrear`, `SedeReemplazo`, `SedeActualizar` |
...
body `{"stock": 99}` muere en 422 contra `SedeReemplazo` y pasa
contra `SedeActualizar`. Cuando todos los verbos comparten forma, la
...
- **Propiedades con `required`**: no se puede construir un `Sede` sin
...
- **La petición declara y el framework valida**: las peticiones por verbo
  (`SedeCrear`, `SedeReemplazo`, `SedeActualizar`) llevan sus
...
1. **El dominio se modela solo:** sede, registro, cliente… son objetos
...
```
Sede (el modelo)             ← la clase entidad: el dato con tipos
SedeCrear / Reemplazo / Actualizar ← las PETICIONES: reglas por verbo (la frontera)
SedeController               ← objeto HTTP; compone un IServicioSede
ServicioSede                 ← objeto de NEGOCIO; compone un IRepositorioSede
IRepositorioSede             ← contrato (interface): abstracción pura
RepositorioSedePostgreSql     ← implementación concreta (encapsula ADO.NET y SQL)
...
4. En este repositorio: [SOLID_CAPAS_PATRONES.md](SOLID_CAPAS_PATRONES.md) y el código
   de `api_catedras/` (comentado línea a línea).
