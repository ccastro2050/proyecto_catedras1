...
- Viene de un **registro** o se construye localmente. Este proyecto usa de
  ...
  `api_catedras/` (base: `dotnet/sdk:10.0`).
...
tiene 2: `./api_catedras`, `./front_flask`.

Este es el de `api-catedras`, sin los comentarios para verlo de un vistazo:

```dockerfile
FROM mcr.microsoft.com/dotnet/sdk:10.0
WORKDIR /app
ENV DOTNET_USE_POLLING_FILE_WATCHER=1 ASPNETCORE_URLS=http://0.0.0.0:8037
EXPOSE 8037
...
| `EXPOSE 8037` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
...
|---|---|---|
| `api-catedras` | **Sí**, en `./api_catedras` | Es código **suyo**: nadie más lo tiene, hay que armarlo |
...
| **Bind mount** | Una carpeta de SU disco montada dentro del contenedor | `./api_catedras:/app` (el código entra al contenedor y `dotnet watch` lo vigila) · `./db:/scripts:ro` (los scripts del init, solo lectura) |
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            FRONTFLASK["SERVIDOR WEB (front)<br/>contenedor front-flask<br/>hostname: front-flask · escucha en 8038"]
            APICATEDRAS["SERVIDOR DE APLICACIONES<br/>contenedor api-catedras<br/>hostname: api-catedras · escucha en 8037"]
            POSTGRES[("SERVIDOR DE BASE DE DATOS<br/>PostgreSQL · contenedor postgres<br/>hostname: postgres · escucha en 5432")]
        end
    end
    NAV -->|"localhost:8037"| APICATEDRAS
    NAV -->|"localhost:8038"| FRONTFLASK
    APICATEDRAS -->|"postgres:5432 (DNS de Docker)"| POSTGRES
    FRONTFLASK -->|"http://api-catedras:8037<br/>al controlador, por el NOMBRE"| APICATEDRAS
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:15461"| POSTGRES
```
...
```yaml
  sqlserver:
    ...
    ports:
      - "15461:1433"                 # "puerto en su PC : puerto interno"
...
```yaml
  sqlserver-init:
    ...
    volumes:
      - ./db:/scripts:ro             # init.sh + catedras_local.sql, solo lectura
...
```yaml
  api-registros:
    build: ./api_catedras            # se construye con SU Dockerfile
    volumes:
      - ./api_catedras:/app          # guardar un .cs → dotnet watch recompila
...
1. **Dos redes de nombres**: hacia su PC, puertos publicados
   (`localhost:8037`, `localhost,15461`); entre contenedores, nombres de
...
docker network create proyecto_catedras1_default

# 2. postgres
docker run -d --name catedras-postgres --network proyecto_catedras1_default --restart unless-stopped `
  -e "POSTGRES_USER=catedras" `
  -e "POSTGRES_PASSWORD=Catedras123!" `
  -e "POSTGRES_DB=catedras" `
  -v pgdata:/var/lib/postgresql/data `
  -v "${PWD}/db/init.sql:/docker-entrypoint-initdb.d/init.sql:ro" `
  -p 15461:5432 postgres:16

# 3. ESPERAR a que responda de verdad… mirándolo a ojo

# 4. Construir la imagen de api-catedras y encenderla
docker build -t api-catedras ./api_catedras
docker run -d --name catedras-api --network proyecto_catedras1_default --restart unless-stopped `
  -e "ConnectionStrings__PostgreSql=Host=postgres;Port=5432;Database=catedras;Username=catedras;Password=Catedras123!" `
  -v "${PWD}/api_catedras:/app" `
  -v /app/bin `
  -v /app/obj `
  -p 8037:8037 api-catedras

# 5. Construir la imagen de front-flask y encenderla
docker build -t front-flask ./front_flask
docker run -d --name catedras-front --network proyecto_catedras1_default --restart unless-stopped `
  -e "API_CATEDRAS_URL=http://api-catedras:8037" `
  -e "CLAVE_SESION=Catedras123!Sesion" `
  -v "${PWD}/front_flask:/app" `
  -p 8038:8038 front-flask
...
```yaml
URL_API: http://api-catedras:8037
```
...
**`api-catedras` es un nombre que solo existe dentro de la red de Docker.**
Su navegador corre en Windows, fuera de esa red, y no sabe quién es.

| Desde dónde | Qué dirección sirve | Por qué |
|---|---|---|
| **Su navegador** | `http://localhost:8037` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-catedras:8037` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8037:8037`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8037`, el front
...
```powershell
docker network inspect proyecto_catedras1_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_catedras1` produce
la red `proyecto_catedras1_default`. Tres formas de confirmarlo:
...
docker inspect catedras-front --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_catedras1_default has active endpoints (name:"catedras-front" …)
```
