...
> 5. **El código pasa a ser un subsede:** si toda la lógica vive en
...
este curso usted lo vive: la [GUIA_IA.md de la versión](spec_kit/versiones/v1_sede/GUIA_IA1.md) construye la versión
...
- **La versión en curso:**
  [spec_kit/versiones/v1_sede/](spec_kit/versiones/v1_sede/2_spec.md)
...
`PATCH /api/sede/{codigo}` con body de la petición SedeActualizar:
...
```markdown
### RF5 — Actualizar parcialmente (PATCH + body parcial)
PATCH /api/sede/{codigo} con campos opcionales: solo se
...
| Tabla | PK | Semilla |
|---|---|---|
| sede | codigo | 8 filas (PR001 "Laptop…", stock 17, …) |
...
```bash
curl http://localhost:8037/api/sede        # total: 8
curl -i http://localhost:8037/api/sede/PR999   # → 404
```
...
- [ ] Modelos/Sede.cs (la entidad: 4 propiedades tipadas)
- [ ] Peticiones/SedeCrear.cs (todo obligatorio, con [Required])
...
```markdown
### RF6 — Eliminar sede
DELETE /api/sede/{codigo} elimina la sede.
...
|---|---|---|---|---|---|
| RF1 — listar | C1, C2 | §2 `GET /api/sede` | Fase 5 · controlador | §2 comando 2 | 1 y 2 |
...
1. La carpeta de la versión es `vN_tema`, en minúsculas y con guion bajo
   (`v1_sede`). El tema dice qué agrega la versión.
...
specify init mi_v1_sede --integration copilot
specify version
```
...
La spec dice:
    RF6 — Eliminar sede
    DELETE /api/sede/{codigo}. Devuelve filasEliminadas;
    inexistente -> 404.

clarify pregunta:
    El borrado de sede, ¿es físico o lógico?
...
clarify NO crea un archivo: escribe dentro de 2_spec.md
    ## Clarificaciones
    C5 — DELETE, ¿físico o lógico? -> Físico: la tabla sede no
...
[la sección de Clarificaciones de 2_spec.md](spec_kit/versiones/v1_sede/2_spec.md),
...
Convergencia — v1
    ...
    Criterio 2  OK    GET /api/sede devuelve las 8 filas semilla
...
[8_tasks.md](spec_kit/versiones/v1_sede/8_tasks.md) —el
cierre— corriendo el smoke test del
[7_quickstart.md](spec_kit/versiones/v1_sede/7_quickstart.md).
...
4. **Es evaluable y comparable.** Los `.md` se leen y se califican, y los
   cursos comparten la misma estructura sobre la misma `catedras_local` con
   stacks distintos.
...
4. En este repositorio: el [spec kit completo](spec_kit/1_constitution.md)
   y la [GUIA_IA.md de la versión](spec_kit/versiones/v1_sede/GUIA_IA1.md) que lo pone a prueba.
