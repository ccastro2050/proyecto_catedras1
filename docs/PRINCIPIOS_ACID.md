...
módulo y carga las filas de la tabla `sede`. Si el motor fallara a la
...
```sql
BEGIN;
UPDATE sede SET activo = 0 WHERE id = …;
...
llave primaria de `sede` impide dos fichas con el mismo `id` —
...
**Dónde se ve aquí:** dos personas abren la pantalla y retiran la mismo
sede al tiempo. Sin aislamiento, las dos leen «está activo» y las dos
...
Un módulo académico está del lado izquierdo: un sede no puede existir a
...
[7_quickstart.md](spec_kit/versiones/v1_sede/7_quickstart.md), que
