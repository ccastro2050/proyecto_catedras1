...
--  Base de datos del EJEMPLO DE REFERENCIA, en UN SOLO script.
-- ==========================================================================
--
--  QUE HACE
--    Crea la base `catedras` desde cero: 37 tablas, 6 vistas, 22 rutinas,
--    7 disparadores, los catalogos, los indices, los cuatro roles y las
--    funciones envoltorio de la API.
--
--  COMO SE EJECUTA
--    No se ejecuta a mano: PostgreSQL lo corre SOLO la primera vez que
--    arranca, porque el compose lo monta en /docker-entrypoint-initdb.d.
--    Para volver a correrlo:  docker compose down -v
--
--  ==========================================================================
--  EN QUE SE DIFERENCIA DEL SCRIPT DADO  (SOLUCION/fisico_postgres/catedras.sql)
--  ==========================================================================
--
--  Este script SE DERIVA de aquel. No es una copia: le quita una cosa y le
--  agrega otra, y las dos estan explicadas aqui.
--
--  1. SE QUITO el bloque 13/18, "Carga REAL del ASIS".               [C1]
--
--     Ese bloque cargaba siete archivos CSV con datos REALES de personas:
--     14.808 asistentes con nombre, apellido y correo institucional, y
--     15.517 numeros de documento de identidad.
--
--     Este repositorio es un EJEMPLO PUBLICO. Publicar ahi la identidad de
--     15.000 personas no es un descuido tecnico: es un problema de
--     proteccion de datos, y ninguna necesidad didactica lo justifica.
--     Para aprender el metodo no hacen falta personas reales.
--
--     Lo que se quito son los DATOS, no el MECANISMO: las tablas de paso
--     (stg_*), el procedimiento de carga y las tablas de lote y novedad
--     siguen aqui, se pueden leer y se pueden ejercitar con datos propios.
--
--  2. SE AGREGO un bloque 13 nuevo: datos HIPOTETICOS.               [C2]
--
--     Pocos y evidentemente inventados: tres programas, dos catedras,
--     cuatro sesiones y cinco asistentes con nombres de ejemplo. Alcanzan
--     para que el sistema tenga algo que mostrar y para que las versiones
--     siguientes tengan sobre que trabajar.
--
--     Que sean POCOS es a proposito: un catalogo de 15.000 filas no enseña
--     nada que no enseñen cinco, y en cambio hace lento cada arranque.
--
--  3. SE ADAPTO el bloque 1: la base la crea Docker, no el script.    [C3]
--
--     Ver la nota en su sitio. En una frase: el compose crea la base y le
--     entrega este archivo ya conectado, asi que el DROP/CREATE DATABASE
--     del original sobra y ademas falla.
--
--  El resto del script —las 37 tablas, los disparadores, las vistas, las
--  rutinas, los roles y los indices— queda TAL CUAL lo entrego el equipo
--  que lo escribio. La base es infraestructura DADA.
--
--  TODO VA EN EL ESQUEMA `public`, como en el original.
--
--  Los informes NO estan aqui: son consultas, no construccion. Estan en
--  material_dado/SOLUCION/fisico_postgres/16-consultas-informes.sql.
-- ==========================================================================
...
-- ---------------------------------------------------------------------------
--  [C3] AQUI ESTABA la creacion de la base: un pg_terminate_backend, un
--  borrado y una creacion de la base, y una reconexion a ella.
--
--  Se quito, y no por gusto: en este proyecto la base la crea DOCKER. El
--  compose le pasa POSTGRES_DB al motor, que crea la base ANTES de ejecutar
--  este archivo -- y lo ejecuta YA CONECTADO a ella. Borrar la base a la
--  que uno esta conectado falla siempre:
--
--      ERROR: cannot drop the currently open database
--
--  El script original suponia que alguien lo corria a mano desde psql,
--  conectado a la base `postgres`. Aqui lo corre el motor solo, la primera
--  vez que arranca con el volumen vacio. Para volver a correrlo:
--
--      docker compose down -v
--
--  Es la misma idea de siempre: la base es infraestructura DADA, y quien la
--  crea es el compose, no el script.
-- ---------------------------------------------------------------------------
...
-- ==========================================================================
--  13/18  ·  Datos HIPOTETICOS
--
--  Reemplaza la "Carga REAL del ASIS" del script dado. Ver [C1] y [C2] en
--  la cabecera: aqui NO hay datos de personas reales.
--
--  Todos los nombres son inventados. Si alguno coincide con el de alguien,
--  es casualidad: no salieron de ningun archivo de la universidad.
-- ==========================================================================

\echo ''
\echo '>>> 13/18 · Datos HIPOTETICOS (no hay datos reales en este ejemplo)'

-- ---------------------------------------------------------------------------
--  Programas academicos
-- ---------------------------------------------------------------------------
INSERT INTO programa_academico (codigo, nombre, fk_facultad, nivel) VALUES
    ('ISIS', 'Ingenieria de Sistemas',      'F_ING',    'PREGRADO'),
    ('IIND', 'Ingenieria Industrial',       'F_ING',    'PREGRADO'),
    ('PSIC', 'Psicologia',                  'F_PSICO',  'PREGRADO')
ON CONFLICT (codigo) DO NOTHING;

-- ---------------------------------------------------------------------------
--  Catedras
--  El id_evento_asis son NUEVE DIGITOS: lo exige chk_catedra_asis (RN-09).
--  Se usa el rango 99xxxxxxx para que se note que son de ejemplo.
-- ---------------------------------------------------------------------------
INSERT INTO catedra (id_evento_asis, nombre, fk_tipo_evento, fk_dependencia) VALUES
    ('990000001', 'Catedra abierta de ejemplo: datos y sociedad', 'CAAB', 'VICE_ACAD'),
    ('990000002', 'Taller de ejemplo: cuidado de la salud mental', 'SLDI', 'SALUD')
ON CONFLICT (id_evento_asis) DO NOTHING;

-- ---------------------------------------------------------------------------
--  Sesiones
--  Se referencian las catedras por su id_evento_asis, no por un numero fijo:
--  la llave la genera la base y no se debe suponer cual salio.
-- ---------------------------------------------------------------------------
INSERT INTO sesion (fk_catedra, numero_reunion, titulo, inicio, fin,
                    fk_modalidad, fk_sede, fk_periodo, lugar)
SELECT c.id_catedra, v.numero, v.titulo, v.inicio, v.fin,
       v.modalidad, v.sede, '2026-1', v.lugar
  FROM (VALUES
        ('990000001', 1::smallint, 'Sesion 1: que son los datos',
         timestamptz '2026-03-03 08:00-05', timestamptz '2026-03-03 10:00-05',
         'PRESENCIAL', 'SAN_BENITO', 'Auditorio 1'),
        ('990000001', 2::smallint, 'Sesion 2: datos y sociedad',
         timestamptz '2026-03-10 08:00-05', timestamptz '2026-03-10 10:00-05',
         'HIBRIDA',    'SAN_BENITO', 'Auditorio 1'),
        ('990000002', 1::smallint, 'Sesion 1: higiene del sueño',
         timestamptz '2026-03-05 14:00-05', timestamptz '2026-03-05 16:00-05',
         'VIRTUAL',    'VIRTUAL',    NULL),
        ('990000002', 2::smallint, 'Sesion 2: manejo del estres',
         timestamptz '2026-03-12 14:00-05', timestamptz '2026-03-12 16:00-05',
         'VIRTUAL',    'VIRTUAL',    NULL)
       ) AS v(evento, numero, titulo, inicio, fin, modalidad, sede, lugar)
  JOIN catedra c ON c.id_evento_asis = v.evento
ON CONFLICT DO NOTHING;

-- ---------------------------------------------------------------------------
--  Asistentes  ·  NOMBRES INVENTADOS
-- ---------------------------------------------------------------------------
--
--  OJO con la ultima fila: la asistente externa NO tiene correo
--  institucional, pero el esquema exige al menos UNA via de contacto
--  (chk_asistente_correo, RN: sin correo no hay como enviarle la clave de
--  acceso). Por eso lleva correo PERSONAL.
--
--  Esa restriccion se comprueba AL INSERTAR, no despues: un INSERT con los
--  dos correos en NULL falla ahi mismo, y no sirve "arreglarlo" con un
--  UPDATE posterior porque la fila nunca llego a entrar.
INSERT INTO asistente (id_asis, nombres, apellidos,
                       correo_institucional, correo_personal, es_externo) VALUES
    ('EJ001', 'Ana Maria', 'Ejemplo Prueba',  'ana.ejemplo@ejemplo.edu.co',     NULL,                       false),
    ('EJ002', 'Carlos',    'Muestra Demo',    'carlos.muestra@ejemplo.edu.co',  NULL,                       false),
    ('EJ003', 'Lucia',     'Ficticia Test',   'lucia.ficticia@ejemplo.edu.co',  NULL,                       false),
    ('EJ004', 'Mateo',     'Inventado Caso',  'mateo.inventado@ejemplo.edu.co', NULL,                       false),
    ('EJ005', 'Sara',      'Externa Visita',  NULL,                             'sara.externa@ejemplo.com', true)
ON CONFLICT DO NOTHING;

-- ---------------------------------------------------------------------------
--  Documentos  ·  NUMEROS INVENTADOS, en el rango de prueba 90000000x
-- ---------------------------------------------------------------------------
INSERT INTO documento_asistente (fk_asistente, fk_tipo_documento, numero)
SELECT a.id_asistente, 'CC', v.numero
  FROM (VALUES ('EJ001', '900000001'), ('EJ002', '900000002'),
               ('EJ003', '900000003'), ('EJ004', '900000004'),
               ('EJ005', '900000005')
       ) AS v(id_asis, numero)
  JOIN asistente a ON a.id_asis = v.id_asis
ON CONFLICT DO NOTHING;

-- ---------------------------------------------------------------------------
--  Matricula de los asistentes internos
-- ---------------------------------------------------------------------------
INSERT INTO programa_asistente (fk_asistente, fk_programa, fk_periodo, estado)
SELECT a.id_asistente, v.programa, '2026-1', 'ACTIVO'
  FROM (VALUES ('EJ001', 'ISIS'), ('EJ002', 'ISIS'),
               ('EJ003', 'IIND'), ('EJ004', 'PSIC')
       ) AS v(id_asis, programa)
  JOIN asistente a ON a.id_asis = v.id_asis
ON CONFLICT DO NOTHING;
...
--  Vinculaciones
--
--  Sin esto el sistema NO deja registrar asistencia: para pedir la clave de
--  acceso, una persona tiene que tener una vinculacion VIGENTE (fecha_fin
--  nula o futura). Es la regla que hace que "quien entra" sea alguien con
--  una relacion real con la universidad, no cualquiera con un enlace.
--
--  Los cuatro internos entran como ESTUDIANTE; la visitante externa, como
--  COLEGIO, que es el unico tipo con exige_programa = false — y por eso ella
--  no aparece en programa_asistente.
-- ---------------------------------------------------------------------------
INSERT INTO vinculacion_asistente (fk_asistente, fk_tipo_vinculacion, fecha_ini)
SELECT a.id_asistente, v.tipo, date '2026-01-19'
  FROM (VALUES ('EJ001', 'ESTUDIANTE'), ('EJ002', 'ESTUDIANTE'),
               ('EJ003', 'ESTUDIANTE'), ('EJ004', 'ESTUDIANTE'),
               ('EJ005', 'COLEGIO')
       ) AS v(id_asis, tipo)
  JOIN asistente a ON a.id_asis = v.id_asis
ON CONFLICT DO NOTHING;

\echo '13 · Datos hipoteticos cargados (5 asistentes, 2 catedras, 4 sesiones)'
